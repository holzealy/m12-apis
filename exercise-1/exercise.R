...
response <- GET('data.unhcr.org/api/stats/time_series_years.json')
body <- content(response, "text")
years <- fromJSON(body)

# What is the "country code" for the "Syrian Arab Republic"?
response <- GET('data.unhcr.org/api/countries/list.json')
countries <- fromJSON(content(response, "text"))
SAR_code <- filter(countries, name_en == 'Syrian Arab Republic') %>% select(country_code)
...
response <- GET('http://www.data.unhcr.org/api/stats/persons_of_concern.json', query = query.params)
...
response <- GET("http://www.data.unhcr.org/api/stats/time_series_all_years.json", query = query.params)
refugess.usa <- fromJSON(content(response, "text"))
refugess.usa <- select(refugess.usa, year, usa = value)
